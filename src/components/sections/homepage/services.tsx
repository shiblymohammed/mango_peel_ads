            ...
            <div ref={cardsRef} className="w-full grid grid-cols-2 gap-2 sm:gap-3 mt-4 sm:mt-8">
              {services.map((service, idx) => (
                ...
                >
                  ...
                  <div className="absolute -bottom-2 -right-1 sm:-bottom-8 sm:-right-4 text-[50px] sm:text-[140px] leading-none font-mileast font-bold text-[#0B132B]/[0.03] lg:group-hover:text-[#0B132B]/[0.1] lg:group-hover:-translate-y-4 lg:group-hover:-translate-x-2 transition-all duration-700 ease-out pointer-events-none select-none z-10">
                    0{idx + 1}
                  </div>

                  {/* Top: Icon & Arrow */}
                  <div className="flex justify-between items-start w-full mb-3 sm:mb-12 relative z-10">
                    <div className="w-6 h-6 xs:w-8 xs:h-8 sm:w-14 sm:h-14 rounded-md sm:rounded-xl border border-[#0B132B]/10 flex items-center justify-center bg-white lg:group-hover:bg-[#0B132B] transition-colors duration-500 shadow-sm">
                       <service.icon className="w-3 h-3 sm:w-6 sm:h-6 text-[#2E7D32] lg:group-hover:text-white transition-colors duration-500" strokeWidth={2.5} />
                    </div>
                  ...
                  <div className="relative z-10 flex flex-col gap-1 sm:gap-3 mt-auto">
                    <h3 className="font-poppins font-bold text-[#0B132B] text-[9px] xs:text-[11px] sm:text-2xl leading-tight">
                      {service.title}
                    </h3>
                    <p className="font-poppins text-[#475569] lg:group-hover:text-[#0B132B]/80 text-[7px] xs:text-[8px] sm:text-[15px] leading-[1.3] sm:leading-relaxed max-w-[95%] transition-colors duration-500">
                      {service.desc}
                    </p>
                  </div>
